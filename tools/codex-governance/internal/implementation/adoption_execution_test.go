...
func TestAdoptPreviewApproveAndReplaySafety(t *testing.T) {
	...
	registryParent, err := os.MkdirTemp(resolvedTemporaryDirectory(t), "adoption-registry-")
	if err != nil {
		t.Fatal(err)
	}
...
func TestValidateScopeDisablesRenameDetectionForCompleteRange(t *testing.T) {
	root, _, _ := testRepository(t)
	writeFile(t, filepath.Join(root, "internal", "old.go"), "package internal\nconst Value = 1\n")
	runGit(t, root, "add", ".")
	runGit(t, root, "commit", "-m", "base")
	base := strings.TrimSpace(runGit(t, root, "rev-parse", "HEAD"))
	runGit(t, root, "mv", "internal/old.go", "internal/new.go")
	runGit(t, root, "commit", "-m", "rename")
	candidate := strings.TrimSpace(runGit(t, root, "rev-parse", "HEAD"))
	runGit(t, root, "config", "diff.renames", "true")
	bundle := TaskBundle{WorkItem: workitem.Item{Scope: workitem.Scope{AllowedPaths: []string{"internal"}, ReviewBudget: workitem.ReviewBudget{MaxChangedFiles: 1, MaxChangedLines: 10, MaxComponents: 1}}}, AllowedPaths: []string{"internal"}}
	if err := validateScope(root, bundle, base, candidate); err == nil || !strings.Contains(err.Error(), "exceeds approved scope budget") {
		t.Fatalf("rename-aware scope evaluation error = %v, want complete-range budget failure", err)
	}
}

func TestPersistAdoptionRetriesPastCrashLeftTemporaryFile(t *testing.T) {
	registry, err := os.MkdirTemp(resolvedTemporaryDirectory(t), "adoption-registry-")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = os.RemoveAll(registry) })
	payloadDigest := "sha256:" + strings.Repeat("a", 64)
	stale := filepath.Join(registry, ".adoption-"+strings.TrimPrefix(payloadDigest, "sha256:")+".tmp")
	if err := os.WriteFile(stale, []byte("crash residue"), 0o600); err != nil {
		t.Fatal(err)
	}
	path := filepath.Join(registry, "record.json")
	if err := persistAdoption(path, signature.Envelope{PayloadDigest: payloadDigest}); err != nil {
		t.Fatalf("persist after crash-left temporary file: %v", err)
	}
