...
func validateScope(dir string, b TaskBundle, base, candidate string) error {
	out, err := git(dir, "diff", "--no-renames", "--numstat", base+".."+candidate)
	if err != nil {
	...
	for _, commit := range strings.Fields(string(commits)) {
		numstat, err := git(dir, "diff-tree", "--no-commit-id", "--no-renames", "--numstat", "-r", commit)
		if err != nil {
			return err
		}
		...
		changed, err := git(dir, "diff-tree", "--no-commit-id", "--no-renames", "--name-only", "-r", commit)
		if err != nil {
			return err
		}
...
func persistAdoption(path string, e signature.Envelope) error {
	...
	temp, err := os.CreateTemp(dir, ".adoption-"+strings.TrimPrefix(e.PayloadDigest, "sha256:")+".tmp-")
	if err != nil {
